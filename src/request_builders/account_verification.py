"""Build requests for account verification API calls"""
import pandas as pd
import datetime
...
def apply_dcc_data_to_verification(request, dcc_context, row):
    """Apply DCC (Dynamic Currency Conversion) data to account verification request"""
    ...
    # For account verification, we use zero amount in merchant currency
    dcc_data.amount = 0  # Always zero for verification
...
def build_account_verification_request(row, cards, address=None, networktokens=None, threeds=None, cardonfile=None, merchantdata=None, previous_outputs=None, dcc_context=None):
    ...
    if pd.notna(row.get('card_entry_mode')):
        ...
        card_payment_data.card_entry_mode = 'ECOMMERCE'  # Default for verification
    ...
    if dcc_context and dcc_context.inverted_exchange_rate:
        apply_dcc_data_to_verification(request, dcc_context, row)
    ...
    # Set amount - always zero for account verification, but use DCC resulting currency if available
    if dcc_context and dcc_context.resulting_amount:
        # Use DCC resulting currency but zero amount
        amount_data = AmountData()
        amount_data.amount = 0  # Always zero for verification
